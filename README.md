...
点“博饼故事”可以阅读四篇文化介绍及官方资料来源；点“博饼规则”可以查看六骰组合图解。全部12种结果都有骰子图片和条件说明，采用本游戏的娱乐版规则，地方习俗可能不同。
...
`verify:pages` 按真实 ECMAScript 模块语义执行已构建的物理 Worker，校验初始化、连续投掷和最终点数。该检查在发布前自动执行，防止普通脚本测试掩盖模块运行错误。
...
- `npm test`：37项通过，包含高处释放取景、100次真实物理投掷、46,656种点数组合穷举、单人停稳后判奖/异常重试、Pages 子路径配置，以及规则图解与实际判奖的一致性。
...
- 原有本地联机网页已通过实际界面操作：创建→准备→开始→投掷→结算→重开→离房。检查1280桌面、390×844竖屏与浏览器错误日志。单人公开版已修复 Ammo 在模块严格模式中的初始化错误，并在真实 Chrome 中完成线上投掷、结算和进入下一轮；控制台无错误。
